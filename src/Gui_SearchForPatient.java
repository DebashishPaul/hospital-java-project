...
public class Gui_SearchForPatient implements ActionListener {
			...
			for (int i = 0; i < Gui_Open.patient.length; i++) {
				if (Gui_Open.patient[i] != null) {
					if (Gui_Open.patient[i].getNid() == x) {
						...
						new View_Patient(Gui_Open.patient[i].getName(), Gui_Open.patient[i].getGender(), nid, AGE,
